...
def test_group_create(client, get_chat_group, new_user):
    index_url = urls.reverse("chat:home")
    client.force_login(user=User.objects.get(username=new_user.get().username))
...
def test_group_accept(client, create_chat_group, new_user):
    ...
    index_url = urls.reverse(
        "chat:accept-membership", kwargs={"name": chat_group.name, "user": user.id}
    )
...
def test_group_deny(client, create_chat_group, new_user, get_user):
    ...
    admin = new_user.get()
    index_url = urls.reverse(
        "chat:deny-membership", kwargs={"name": chat_group.name, "user": user.id}
    )
    Member.objects.create(group=chat_group, user=user, accepted=False)
    chat_group.admin.add(admin.id)
    client.force_login(user=User.objects.get(username=admin.username))
...
def test_group_revoke(client, create_chat_group, new_user):
    chat_group = create_chat_group
    user = new_user.get()
    admin = new_user.get()
    index_url = urls.reverse(
        "chat:revoke-membership", kwargs={"name": chat_group.name, "user": user.id}
    )
    Member.objects.create(group=chat_group, user=user, accepted=True)
    chat_group.admin.add(admin.id)
    client.force_login(user=User.objects.get(username=admin.username))
    resp = client.get(index_url)
    assert resp.status_code == 302
