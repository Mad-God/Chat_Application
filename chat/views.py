...
class HomeView(View):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context["joined_groups"] = ChatGroup.objects.annotate(
                mem_count=Count("members")
            ).filter(
                members__user=self.request.user, members__accepted=True, mem_count__gt=0
            )
            context["applied_groups"] = ChatGroup.objects.annotate(
                mem_count=Count("members")
            ).filter(
                members__user=self.request.user,
                members__accepted=False,
                mem_count__gt=0,
            )
            context["other_groups"] = (
                ChatGroup.objects.annotate(mem_count=Count("members"))
                .exclude(members__user=self.request.user, members__gt=0)
                .filter(mem_count__gt=0)
            )
        else:
            context["other_groups"] = ChatGroup.objects.annotate(
                mem_count=Count("members")
            ).filter(mem_count__gt=0)
...
class DirectLobby(LoginRequiredMixin, View):
    ...
    def get(self, args, **kwargs):
        ...
        return render(
            self.request,
            "chat/lobby.html",
            {"group": group, "previous_messages": messages, "form": form},
        )
...
class ApplyForMemberShip(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        Member.objects.create(
            ...
            group=ChatGroup.objects.get(slug=kwargs["name"]),
            accepted=False,
        )
